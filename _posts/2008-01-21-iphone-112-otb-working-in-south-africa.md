...
toc: false
classes: wide
excerpt: After seeing Colin's iPhone working in SA I decided to buy one myself. The only issue is that all the new iPhones shipped with version 1.1.2 firmware on it can be hacked but there is no way to unlock the SIM card feature on these new versions.
